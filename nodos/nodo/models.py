...
class Vehicle(models.Model):
    ...
    fuel_efficiency = models.DecimalField(max_digits=6, decimal_places=2, blank=True, null=True) # in km/L
    fuel_tank_size = models.DecimalField(max_digits=6, decimal_places=2, blank=True, null=True)
...
class ServiceArea(models.Model):
    ...
    def __str__(self) -> str:
        return f"{self.kilometer} (gas price: {self.gas_price})"

    def get_parent(self):
        return ServiceArea.objects.get(Q(left_station_id=self.id) | Q(right_station_id=self.id))

    def get_left(self):
        return self.left_station

    def get_right(self):
        return self.right_station
